...
class Panel extends h2d.Bitmap
{
	/**
	 *  Mask
	 */
	public var panelOverflow(default, set): PanelOverflow = Visible;
	var parentMask : Mask;
	...
	**/
	...
	function set_panelOverflow( v : PanelOverflow ) : PanelOverflow
	{
		if( v != panelOverflow )
			updateMask();

		return panelOverflow = v;
	}
...
@:uiComp("panel")
class PanelComp extends h2d.domkit.BaseComponents.BitmapComp  implements domkit.Component.ComponentDecl<Panel>
{
	@:p(auto) var background_color : Null<String>;

	static function create( parent : h2d.Object ) {
		return new Panel(parent);
	}

	static function set_background_color( o : Panel, t ) {
		o.tile = h2d.Tile.fromColor(  ,32,32,0.9) ;
	}
