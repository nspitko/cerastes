...
class ImGuiTools {
  ...
  public static function textInput(label: String, value: String, ?length: Int = 1024): Null<String>
	{
		var textBuf = new hl.Bytes(1024);
		var src = haxe.io.Bytes.ofString(value);
		textBuf.blit(0,src,0,value.length);
		textBuf.setUI8(value.length,0); // Null term

		if (ImGui.inputText(label, textBuf, 1024)) {
