...
class AffineFog extends hxsl.Shader {
    static var SRC = {
		...
		@param var fogNear: Float;
		@param var fogFar: Float;
...
//				var distance = length(  projectedPosition * global.modelView.mat3x4() );
			...
			function fragment()
			{
				...
				var fogStrength = smoothstep(fogNear, fogFar, fogDepth);

				pixelColor = mix(pixelColor, fogColor, fogStrength);
			}
    };
