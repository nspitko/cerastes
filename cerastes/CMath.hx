// Originally had my own code here, but replaced with https://github.com/deepnight/deepnightLibs/blob/master/src/dn/M.hx
/*
...
import haxe.macro.Expr;
 ...
 class CMath {
	 ...
	 inline public static function signEq(x:Float,y:Float):Bool {
		 return CMath.sign(x)==CMath.sign(y);
	 }
	 ...
	 **/
	 ...
	 inline public static function randRange(min:Int, max:Int, ?rnd:Void->Float):Int
	 {
		 var l = min - .4999;
		 var h = max + .4999;
		 return CMath.round(l + (h - l) * frand(rnd));
	 }
	  ...
	  * Donne des résultats différents de budum9.NuCMath.hMod mais devrait fonctionner en gros de la meme façon.
	  */
	 ...
	 public static inline function radCloseTo(curAng:Float, target:Float, maxAngDist:Float) {
		 return radDistance(curAng, target) <= CMath.fabs(maxAngDist);
	 }
