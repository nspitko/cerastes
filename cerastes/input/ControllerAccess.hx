package cerastes.input;

import hxd.Pad;
import hxd.Key;
import cerastes.input.Controller;
import cerastes.CMath as M;

// https://github.com/deepnight/deepnightLibs/tree/master/src/dn/heaps/input
...
class ControllerAccess<T:EnumValue> {
	...
	@:allow(cerastes.input.Controller)
	...
	**/
	public inline function getAnalogDistXY(xAction:T, ?yAction:T, clamp=true) {
		inline function _dist() {
			return M.dist( 0, 0, getAnalogValue(xAction), getAnalogValue(yAction) );
		}
		return
			yAction==null
				? M.fabs( getAnalogValue(xAction) )
				: clamp
					? M.fmin( _dist(), 1 )
					: _dist();
	}
	...
	**/
	public inline function getAnalogDist4(leftAction:T, rightAction:T, upAction:T, downAction:T, clamp=true) : Float {
		inline function _dist(){
			return M.dist( 0,0, getAnalogValue(leftAction)+getAnalogValue(rightAction), getAnalogValue(upAction)+getAnalogValue(downAction) );
		}
